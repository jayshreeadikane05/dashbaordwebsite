        ...
        <div class="col-8 col-lg-8">
            ...
            <div class="card">
                <div class="card-body p-4">
                    ...
                    <h5 class="mb-4">Add Video</h5>
                    <form id="addVideoForm" enctype="multipart/form-data">
                        @csrf
                        <div class="row mb-3">
                            <div class="col-sm-12">
                                <label for="blogvideo" class="col-sm-3 col-form-label">Feature Video</label>
                                <div id="drop-area">

                                <input type="file" id="blogvideo" name="video" class="file-input" accept="video/*" />

                                    <img src="{{ asset('assets/images/drap-drop.png') }}" class="img-fluid" alt="">
                                    <label for="blogvideo" class="drag-drop-label">Drag & Drop or Click to
                                        Upload</label>
                                        <p>Only mp4, mov, avi, and video files up to 50 MB are allowed.</p>
                                        <label for="blogvideo" class="drag-drop-label" id="button">Upload Files</label>


                                        <video id="videoPreview" controls style="display:none; margin-top:10px; max-height: 200px;">
                                        <source src="#" type="video/mp4">
                                        Your browser does not support the video tag.
                                    </video>
                                    <div style="margin-top:10px; display:none;" id="videoAction">
            <button type="button" id="replaceVideo" class="btn btn-warning btn-sm">Replace</button>
            <button type="button" id="removeVideo" class="btn btn-secondary btn-sm">Remove</button>
        </div>
                                </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-sm-12">
                                <label for="blogname" class="col-sm-3 col-form-label">Name*</label>
                                <input type="text" class="form-control" name="videoname" id="blogname"
                                    placeholder="Add Video Name" required>
                            </div>
                        </div>

                        <div class="row mb-3">
                            <div class="col-sm-12">
                                <label for="selectedCategories" class="col-sm-3 col-form-label">Selected
                                    Categories</label>
                        ...
                        <div class="row mb-3">
                            <div class="col-sm-12">
                                <label for="description" class="col-sm-3 col-form-label">Video Description</label>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="d-md-flex d-grid align-items-center gap-3">
                                    <button type="button" id="submitBtn" class="btn btn-primary px-4">Publish</button>
                            ...
                            <div class="col-sm-12 categories">
                                ...
                                <div class="categories-box">
                                    ...
                                    <div class="category-item" data-id="{{ $category->id }}">
                                        <span class="category-label " draggable="true" data-id="{{ $category->id }}">
                                            {{ $category->name }}
                                            <i class="fa fa-pencil edit-category"
                                                style="margin-left: 10px; cursor: pointer;"
                                                data-id="{{ $category->id }}" title="Edit"></i>
                                            ...
                                            <span class="delete-category-icon" data-id="{{ $category->id }}"
                                ...
                                <div class="d-flex justify-content-center">
                                    <button type="button" class="btn " data-bs-toggle="modal"
                                        data-bs-target="#addCategoryModal"> <img
                                            src="{{ asset('assets/images/Group.svg') }}" class="img-fluid" alt=""> Add
                                        Category</button>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-12 categories">
                                ...
                                <div class="tags-box">
                                    ...
                                    <span class="tag-label" draggable="true" data-id="{{ $tag->id }}">
                                        {{ $tag->name }}
                                        <i class="fa fa-pencil edit-tag" style="margin-left: 10px; cursor: pointer;"
                                            data-id="{{ $tag->id }}" title="Edit"></i>
                                        <form class="delete-tag-form" data-id="{{ $tag->id }}"
                                            action="{{ route('tag.delete', $tag->id) }}" method="POST"
                                            style="display:none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                ...
                                <div class="d-flex justify-content-center">
                                    <button type="button" class="btn " data-bs-toggle="modal"
                                        data-bs-target="#addTagModal"><img src="{{ asset('assets/images/Group.svg') }}"
                                            class="img-fluid" alt=""> Add Tag</button>
                                </div>
...
<div class="modal fade" id="confirmTagModal" tabindex="-1" role="dialog" aria-labelledby="confirmModalTagLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
...
$(document).on('click', '.edit-category', function() {
    ...
    const categoryName = $(this).closest('.category-item').find('.category-label').contents().filter(
function() {
        ...
        success: function(response) {
            if (response.success) {
                ...
                if (!exists) {
                    ...
                    ${response.tag.name}
                    <i class="fa fa-pencil edit-tag" style="margin-left: 10px; cursor: pointer;" data-id="${response.tag.id}" title="Edit"></i>
...
$(document).ready(function() {
    ...
    $('#reset').on('click', function(e) {
        e.preventDefault();
        $('#addVideoForm input').val('');
        $('#addVideoForm input[type="checkbox"]').prop('checked', false);
    ...
    document.getElementById('replaceVideo').addEventListener('click', function() {
    document.getElementById('blogvideo').click(); 
});
...
document.getElementById('removeVideo').addEventListener('click', function() {
    ...
    const fileInput = document.getElementById('blogvideo');

    fileInput.value = '';  
    videoPreview.src = '#';  // Reset the video preview
    videoPreview.style.display = 'none';  // Hide video preview
    videoAction.style.display = 'none';  // Hide action buttons
});

    document.getElementById('blogvideo').addEventListener('change', function(event) {
    ...
    function dropTag(event) {
        ...
        if (!addSelectedItem('selectedTags', id, name)) {
            ...
            toastr.error("This tag is already added.");
        } else {
    ...
    function addHiddenInput(type, id) {
        $(`#addVideoForm`).append(`<input type="hidden" name="${type}[]" value="${id}">`);
    }
    ...
    $('#submitBtn').on('click', function(e) {
        ...
        if ($('#addVideoForm')[0].checkValidity() === false) {
            e.stopPropagation();

            $('#addVideoForm').find(':input[required]').each(function() {
